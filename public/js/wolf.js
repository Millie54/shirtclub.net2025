...
function showResult(rank, source){
	emptySearchResult();
	if(rank.length === 0){
		$('section#results').append('<h4>No Results Found</h4>');
	}
	else{
		var tableBuilder = '<h4>' + source + ' Ranking Results:</h4><br><table class="table"><tr><th>Rank</th><th>Player Name</th></tr>'
 ...
 */
 $('.searchWorld').on('click', function(){
	...
	$.ajax({
		...
		if(data.success){
			showResult(data.rank, 'World');
		}
		else{
			var rank = [];
			showResult(rank, 'World');
		}
