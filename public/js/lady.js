...
function showResult(rank){
	...
	$.ajax({
		...
		if(data.success){
			...
			var females = data.results;

			if(females.length === 0 || rank.length === 0){
				...
				for(var i = 0; i < rank.length; i++){
					var playerName = rank[i][0];
					var playerScore = rank[i][1];
					for(var j = 0; j < females.length; j++){
						if(playerName === females[j].user_name){
							newRank.push([playerName, playerScore]);
						}
					}
				}
				...
				if(newRank.length === 0){
					...
					var top = newRank.length > 100 ? 100 : newRank.length;
					var rankIndex = 1;
					var skipIndex = 0;
					for(var i = 0; i < top; i++){
						// Same score as previous ranking score
						if(i > 0 && newRank[i-1][1] != newRank[i][1]){
							rankIndex+=skipIndex;
							skipIndex = 0;
							rankIndex++;
						}
						...
						tableBuilder += '<td>' + newRank[i][0] + '</td>';
						tableBuilder += '</tr>';
					}
					tableBuilder += '</table>';
